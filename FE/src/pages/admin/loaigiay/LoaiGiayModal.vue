  ...
  <a-modal :open="open"  @cancel="closeModal" :title="props.title" width="400px">
    ...
    <a-form :model="product" ref="productForm" name="productForm" autocomplete="off">
      <a-form-item label="Tên danh mục" name="ten" :label-col="{ span: 24 }" :rules="rules.ten">
        <a-input v-if="product" v-model:value="product.ten" placeholder="Nhập tên danh mục" style="border-radius: 4px;" />
      </a-form-item>
    </a-form>
  </a-modal>
</template>
...
const rules = {
  ten: [{ required: true, message: 'Tên danh mục không được để trống!', trigger: 'blur' }],
};
