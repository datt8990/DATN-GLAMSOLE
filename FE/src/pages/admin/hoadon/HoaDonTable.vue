<template>
  ...
  <a-tag :color="
  record.status === 'CHO_XAC_NHAN' ? 'orange' :
  record.status === 'DA_XAC_NHAN' ? 'gold' :
  record.status === 'CHO_GIAO' ? 'blue' :
  record.status === 'DANG_GIAO' ? 'cyan' :
  record.status === 'XAC_NHAN_THANH_TOAN' ? 'purple' :
  record.status === 'HOAN_THANH' ? 'green' :
  record.status === 'DA_HUY' ? 'red' : 'default'
">
  {{
    record.status === 'CHO_XAC_NHAN' ? 'Chờ xác nhận' :
    record.status === 'DA_XAC_NHAN' ? 'Đã xác nhận' :
    record.status === 'CHO_GIAO' ? 'Chờ giao' :
    record.status === 'DANG_GIAO' ? 'Đang giao' :
    record.status === 'XAC_NHAN_THANH_TOAN' ? 'Xác nhận thanh toán' :
    record.status === 'HOAN_THANH' ? 'Hoàn thành' :
    record.status === 'DA_HUY' ? 'Đã hủy' : 'Không rõ'
  }}
</a-tag>
</template>
