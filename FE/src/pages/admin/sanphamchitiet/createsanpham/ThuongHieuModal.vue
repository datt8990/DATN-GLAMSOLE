...
import { getThuongHieu, type ThuongHieuResponse, modifyThuongHieu } from '@/services/api/admin/thuonghieu.api';
import { toast } from 'vue3-toastify';

const props = defineProps<{ open: boolean; productId: string | null; title: string }>();
const emit = defineEmits(['close', 'success']);

const product = ref<ThuongHieuResponse>({ ten: '', mau: '#000000', ma: '', id: '' });
const productForm = ref();

const rules = {
  ten: [{ required: true, message: 'Thương hiệu không được để trống!', trigger: 'blur' }],
};

const generateCode = () => {
  const randomNumber = Math.floor(1000 + Math.random() * 9000); // đảm bảo luôn 4 chữ số
  return 'TH_' + randomNumber;
};

const fetchProductDetails = async (id: string) => {
  try {
    const response = await getThuongHieu(id);
