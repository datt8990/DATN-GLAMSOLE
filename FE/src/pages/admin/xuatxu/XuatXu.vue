...
import ProductFilter from './XuatXuFilter.vue';
import ProductTable from './XuatXuTable.vue';
import ProductModal from './XuatXuModal.vue';
import { computed, onMounted, reactive, watch } from 'vue';
import { GetXuatXus, type XuatXuResponse, type ParamsGetXuatXu } from '@/services/api/admin/xuatxu.api';
...
const modalTitle = computed(() => {
  return state.selectedProductId ? 'Cập nhật xuất xứ' : 'Thêm xuất xứ'
})
...
const fetchProducts = async () => {
  try {
    const params: ParamsGetXuatXu = {
      page: state.paginationParams.page,
      size: state.paginationParams.size,
      q: state.searchQuery,
      status: state.searchStatus
    }
    const response = await GetXuatXus(params)
