<template>
    <DivCustom label="Danh sách xuất xứ" customClasses="mt-5">
        <template #extra>
            <a-tooltip title="Thêm xuất xứ">
...
import { modifyStatusXuatXu } from '@/services/api/admin/xuatxu.api'
...
const columns: TableColumnsType = [
    ...
    { title: 'Mã xuất xứ', key: 'ma', dataIndex: 'ma', width: 50, align: 'center' },
    { title: 'Tên xuất xứ', key: 'ten', dataIndex: 'ten', width: 150, align: 'center' },
...
const handleChangeStatusClick = async (id: string) => {
    try {
        const res = await modifyStatusXuatXu(id);
        emit('changeStatus');
