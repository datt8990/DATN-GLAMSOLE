<template>
  <a-modal :open="open" :title="props.title" width="400px">
    ...
    <a-form :model="product" ref="productForm" name="productForm" autocomplete="off">
      <a-form-item label="Tên chất liệu" name="ten" :label-col="{ span: 24 }" :rules="rules.ten">
        <a-input v-if="product" v-model:value="product.ten" placeholder="Nhập chất liệu" style="border-radius: 4px;" />
      </a-form-item>
    </a-form>
  </a-modal>
</template>
...
import { getSize, type SizeResponse, modifySize } from '@/services/api/admin/chatlieu.api';
